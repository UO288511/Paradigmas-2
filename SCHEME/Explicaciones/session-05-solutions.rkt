...
(define (filter-for f . rest)
  (map (curry filter f) rest))
...
(define (info key p)
  (cdar (filter (lambda(x) (equal? (car x) key)) p)))
...
;; también se podría definir con compose y curry:
(define (info key p)
  (cdar (filter (compose (curry equal? key) car) p)))

(display "info (compose-curry): ")
(info 'apellidos (cadr Datos))  ; => (LUZ DIVINA)

;; Define la función buscar(key datos) que retorna la lista con la información de
...
(define (info+ l-keys p)
  (apply append (map (lambda(key)
                 (info key p))
               l-keys)))

(define (buscar+ l-keys datos)
  (map (curry info+ l-keys) datos))

;(define (buscar+ l-keys datos)
;  (map (curry (lambda(l-keys p)
;                (apply append
;                       (map (lambda(key)
;                              (info key p))
;                            l-keys)))
...
(define (presentar Datos orden_claves)
  (map (lambda(item) 
         (sort item (lambda(x y)
                      (not (member (car x)
                                   (member (car y) orden_claves)))))) Datos))
...
;  (JOSHUA IGLESIAS GARCIA 12 V MEDIOS #t)
...
(define (change-all l u v)
  (map (lambda(elem)
         (if (equal? elem u) v elem)) l))
...
; 1. Base       : fill(x, ()) = (x)
; 2. Recurrencia: la lista l no es vacía => l = cons(car(l), cdr(l))
;      Hipótesis: se conoce fill(x, cdr(l)) = H
;          Tesis: fill(x, l) = cons(cons(x, car(l)),
;                                   map(y -> cons(car(l), y), H))

(define (fill x l)
  (if (null? l)
      (list (list x))
      (cons (cons x l) (map
                        (lambda(y) (cons (car l) y))
                        (fill x (cdr l))))))
...
(define (fill x l)
  (if (null? l)
      (list (list x))
      (cons (cons x l) (map (curry cons (car l)) (fill x (cdr l))))))

(display "fill (curry): ")
(fill 'x '(a (b c) d)) ; => ((x a (b c) d) (a x (b c) d) (a (b c) x d) (a (b c) d x))

;;-------------------------------------------------------------------------
;; Definir la función my-curry para que se comporte igual que la
;; currificación de una función f(x, y) de dos argumentos.
;;
...
(define (my-curry f . args)
  (cond [(> (length args) 2)
         (error "my-curry: requiere a lo sumo 2 argumentos")]
        [(= (length args) 1)
         (lambda(x)
           (f (car args) x))]
        [else
