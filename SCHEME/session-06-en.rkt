...
(define (compatibles card hand)
  (filter (lambda (x)
            (or (color-compatible (cdr card) hand)        
                (value-compatible (car card) hand)))    
          hand))
...
; WRITE THE DEFINITION HERE
(define (players-with-wild hands)
  (map car(filter (lambda (player)
            (wildcard? (player-hand (car player) hands)))  
          hands)))
...
(apply + (apply append (map (lambda (x) (if (list? x) x '())) Vectors)))
